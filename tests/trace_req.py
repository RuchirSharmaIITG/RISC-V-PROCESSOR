...
vars = { 'stall_ex_request': None, 'mult_div_en_i': None, 'mult_div_ready': None }
...
with open(vcd_path, 'r') as f:
    ...
    for line in f:
        ...
        if 740000 <= time <= 760000 and line.startswith('#'):
            print(f"Time {time}: stall_req={state['stall_ex_request']} mul_en={state['mult_div_en_i']} ready={state['mult_div_ready']}")
